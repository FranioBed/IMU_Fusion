...
void main() {
	...
	PRIO_ERROR err = prio_getPort(port.port_name, com_offset, &port.device_type);
	printf("Get port result: %s\n", err);
	// Create a PrioVR Basestation (was Hub) object from the communication port
	err = prio_createBaseStation(port.port_name, &bs_device);
	printf("create basestation result: %d", err);
	// Get the PrioVR Hub paired to the PrioVR Basestation
	err = prio_bs_getWirelessHub(bs_device, &hub_device);
	printf("create hub result: %d", hub_device);
	...
	prio_bs_startRecording(bs_device, PRIO_STREAM_ALL_RAW_COMPONENT_SENSOR_DATA, 0,
		PRIO_STREAM_DURATION_INFINITE);
	...
	for (int i = 0; i < (int)packet_count; i++) {
		...
		int sensorDataSize = 36;
		int packetSize = 684;
		for (int j = 0; j < 19; j++) {
			// Get the current sensor quaterion
			float tmp[9];
			memcpy(tmp, packetData + (i * packetSize) + j * sensorDataSize, sensorDataSize);
			fprintf(fp, "(%f,\t %f,\t %f),\t\t (%f,\t %f,\t %f),\t\t (%f,\t %f,\t %f)\ n", tmp[0], tmp[1], tmp[2], tmp[3], tmp[4], tmp[5], tmp[6], tmp[7], tmp[8]);
		}
