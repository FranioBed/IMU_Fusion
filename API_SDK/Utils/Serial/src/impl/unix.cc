...
void serial::WaitForMultiplePorts(std::vector<std::shared_ptr<Serial>>& ports)
{
    ...
    {
        if( (*it)->pimpl_->getFD() > highestfd )
        {
            highestfd = (*it)->pimpl_->getFD();
        }
        FD_SET((*it)->pimpl_->getFD(), &readfds);
    }

    // Call select to block for serial data or a timeout
    int retval = select(highestfd+1, &readfds, NULL, NULL, &tv);
}

int Serial::SerialImpl::getFD()
{
    return fd_;
}
