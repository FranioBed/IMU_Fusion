    ...
    bytesize_(bytesize), stopbits_(stopbits), flowcontrol_(flowcontrol)
{
    ...
    read_asynchronous = { 0 };
    read_asynchronous.hEvent = CreateEvent(NULL, TRUE, FALSE, NULL);
    write_asynchronous = { 0 };
    write_asynchronous.hEvent = CreateEvent(NULL, TRUE, FALSE, NULL);
...
Serial::SerialImpl::~SerialImpl()
{
    ...
    CloseHandle(read_asynchronous.hEvent);
    CloseHandle(write_asynchronous.hEvent);
}
...
Serial::SerialImpl::open()
{
    ...
    // Check if COM port number greater than 9
    if (!port_.compare(0, 3, L"COM"))
    {
        if (std::stoi(port_.substr(3)) > 9)
        {
            port_.insert(0, L"\\\\.\\");
        }
    }
...
Serial::SerialImpl::read(uint8_t *buf, size_t size)
{
    ...
    ResetEvent(read_asynchronous.hEvent); //might be able to remove? 

    DWORD bytes_read;
    if (!ReadFile(fd_, buf, size, &bytes_read, &read_asynchronous)) {
    ...
    if (!GetOverlappedResult(fd_, &read_asynchronous, &bytes_read, TRUE)){
        stringstream ss;
        ss << "Error while reading from the serial port: " << GetLastError();
        THROW(IOException, ss.str().c_str());
    }
...
Serial::SerialImpl::write(const uint8_t *data, size_t length)
{
    ...
    ResetEvent(write_asynchronous.hEvent); //might be able to remove? 
    DWORD bytes_written;
    if (!WriteFile(fd_, data, length, &bytes_written, &write_asynchronous)) {
        DWORD last_error = GetLastError();
        if (last_error != ERROR_IO_PENDING){
            stringstream ss;
            ss << "Error while writing to the serial port: " << last_error;
            THROW(IOException, ss.str().c_str());
        }
    }
    if (!GetOverlappedResult(fd_, &write_asynchronous, &bytes_written, TRUE)){
        stringstream ss;
        ss << "Error while reading from the serial port: " << GetLastError();
        THROW(IOException, ss.str().c_str());
    }
...
void serial::WaitForMultiplePorts(std::vector<std::shared_ptr<Serial>>& ports)
{
    std::vector<HANDLE> port_handles;

    for (std::vector<std::shared_ptr<Serial>>::iterator it = ports.begin(); it != ports.end(); ++it)
    {
        port_handles.push_back((*it)->pimpl_->read_asynchronous.hEvent);
    }

    DWORD error = WaitForMultipleObjects(port_handles.size(), port_handles.data(), false, 100);
}
#endif // #ifdef _WIN32